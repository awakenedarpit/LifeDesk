...
export const BottomNav: React.FC = () => {
  ...
  const navItems: { id: AppNavTab; label: string; icon: string; badge?: number }[] = [
    ...
    { id: 'hackathons', label: 'Sprints', icon: 'terminal' },
    { id: 'money', label: 'Finances', icon: 'account_balance_wallet' },
  ];

  return (
    <nav className="md:hidden fixed bottom-0 w-full z-40 pb-safe bg-surface/95 backdrop-blur-xl border-t border-surface-container shadow-[0_-2px_12px_rgba(0,0,0,0.06)]">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center h-full py-1 transition-all group relative select-none touch-manipulation min-w-[56px] ${
                isActive
            ...
            >
              <div className="relative flex items-center justify-center">
                ...
                {item.badge !== undefined && (
                  <span className="absolute -top-1 -right-2 min-w-[16px] h-4 px-1 rounded-full bg-error text-on-error font-label-xs text-[9px] font-bold flex items-center justify-center">
                    {item.badge}
                  </span>
                )}
              </div>

              <span className={`font-label-xs text-[10px] mt-1 transition-all ${
